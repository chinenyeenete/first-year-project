...
let playerCoins = 0;
let compCoins = 0;
let numOfCoins = Math.floor(Math.random() * 5) * 2 + 1;
const coinsToWin = Math.ceil(numOfCoins / 2);
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    function decideCoinLocations() {
      ...
      while (coinCollection.length < numOfCoins) {
  ...
  determineWinner() {
    if (playerCoins === coinsToWin || compCoins === coinsToWin) {
      this.endGame();
    }
  }
} // End GameScene
