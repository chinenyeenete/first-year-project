...
class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    const applyHitReward = () => {
      ...
      const timePressed = lastHitTime - timeLeft;
	  const constant = Math.ceil((10-Math.floor(timePressed/3*10))/2);
      const multiplier = timePressed < 3 ? constant : 1;
      updateScore(10*multiplier);
      this.displayRewardText(multiplier);
  ...
  initializeTimer(timerCallback) {
    gameState.timerText = this.add
      .text(50, 75, `Time: ${Number.parseFloat(timeLeft).toFixed(2)}`)
      .setColor("#000000");

    this.time.addEvent({
      delay: 10, // call after every 1000ms (1sec)
      callback: timerCallback, // run function after 1sec elapsed
      args: [0.01], // amount of time that elapsed
  ...
  updateTimerText() {
    gameState.timerText.setText(
      `Time: ${Number.parseFloat(timeLeft).toFixed(2)}`
    );
  }
  ...
  displayRewardText(multiplier) {
    // add text to display score reward
    const rewardText = this.add.text(160, 50, `+10 x ${multiplier}`).setColor("#228B22");
    this.time.addEvent({
  ...
  displayPenaltyText() {
    ...
    const penaltyText = this.add.text(160, 50, "-10").setColor("#991A00");
    this.time.addEvent({
